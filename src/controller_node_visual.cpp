...
class controller_node_visual : public rclcpp::Node {
  ...
  bool has_velocity_setpoint_ = false;
...
public:
  controller_node_visual()
  : Node("controller_node_visual"),
    ...
    yawd(0.0), e3(0, 0, 1), hz(100)
  {
    ...
    timer_ = this->create_wall_timer(
      ...
      std::bind(&controller_node_visual::controlLoop, this));
  ...
  {
      vd_from_servo_ << msg->twist.linear.x,
                        msg->twist.linear.y,
                        msg->twist.linear.z;
      has_velocity_setpoint_ = true;

      // Optional: clamp or sanity check
      if (vd_from_servo_.norm() > 2.0) {
          vd_from_servo_ = vd_from_servo_.normalized() * 2.0;
      }
  }
  ...
  void controlLoop() {
    ...
    if (has_velocity_setpoint_) {
        // Set desired position = current position => position error = 0
        xd = x;
        // Desired velocity = from visual servo
        vd = vd_from_servo_;
        // Desired acceleration = 0
        ad.setZero();
    } else {
        // Original behaviour: ramp altitude to 2m and hold XY
        ...
        if (xd_z_ramp_ < 2.0) {
            xd_z_ramp_ = std::min(xd_z_ramp_ + CLIMB_RATE / hz, 2.0);
        }
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::spin(std::make_shared<controller_node_visual>());
  rclcpp::shutdown();
  return 0;
}